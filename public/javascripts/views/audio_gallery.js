...
$.jstree._themes = "/images/jstree/themes/";
  ...
  $("#audioFileTree").jstree({
    ...
    "json_data" : {
        "ajax" : {
        "url" : "/ajax/show_audio",
        "data" : {"project_id" : project_id, "path" : "sound"}
        }
...
$("#audioFileTree").bind("select_node.jstree", function(event, data) {
   selected_file = data.rslt.obj.data("jstree").path;
   selected_type = data.rslt.obj.data("jstree").type;
   if(selected_type == "file") {
       niftyplayer('audioPlayer').load("/projects/" + project_id + "/" + selected_file);
   }
   else {
       $("#targetFolder").val(selected_file);
       $("#audioUploadFolder").text("Folder: " + selected_file);
       niftyplayer('audioPlayer').stop();
   }
});

$("#audioFileTree").bind("move_node.jstree", function(event, data) {
    /*
...
$("#audioFileTree").bind("rename_node.jstree", function(event, data) {
...
$(document).ready(function() {
 $("#audioActions").accordion();
});

$(document).ready(function() {
    $("#allProjectsAudioFileTree").jstree({
    ...
    $("#allProjectsAudioFileTree").bind("select_node.jstree", function(event, data) {
   selected_file = data.rslt.obj.data("jstree").path;
   selected_type = data.rslt.obj.data("jstree").type;
   selected_project_id = data.rslt.obj.data("jstree").project_id;

   if(selected_type == "file") {
       $("#audioPreview").attr("src","/projects/" + selected_project_id + "/" + selected_file);
   }
   else {
...
function deleteImageFile(path) {
    ...
    $.ajax({
        ...
        success : function(data) {
            if(data.length > 0) {
                text = "This audio file is used in at least one mission.\n" +
